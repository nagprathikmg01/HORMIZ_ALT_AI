  ...
  return (
    <header className={`sticky top-0 z-50 transition-all duration-300 border-b backdrop-blur-xl ${
      ...
      <div className={`border-b px-4 py-1.5 text-xs transition-colors ${
        isDark ? 'bg-gradient-to-r from-red-950 via-slate-900 to-red-950 border-red-800/40 text-slate-200' : 'bg-red-50 border-red-200 text-red-900'
      }`}>
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-semibold text-red-500 tracking-wide uppercase">Live Disruption Engine:</span>
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center space-x-3 cursor-pointer" onClick={() => scrollToSection('hero')}>
            <div className={`h-9 w-9 rounded-xl p-0.5 shadow-sm border ${
              isDark ? 'bg-slate-900 border-sky-500/40 text-sky-400' : 'bg-sky-50 border-sky-300 text-sky-600'
            ...
            <div>
              <div className="flex items-center space-x-2">
                ...
                <span className={`text-[10px] font-mono font-bold px-1.5 py-0.5 rounded border ${
                  isDark ? 'bg-sky-950 border-sky-500/40 text-sky-300' : 'bg-sky-100 border-sky-300 text-sky-700'
                }`}>AI</span>
              </div>
              ...
              return (
                <button
                  ...
                  className={`flex items-center space-x-1.5 px-2.5 py-2 rounded-lg text-xs font-medium transition-all ${
                    isActive 
                      ? isDark ? 'bg-sky-950 text-sky-300 border border-sky-500/40 font-bold' : 'bg-sky-50 text-sky-700 border border-sky-200 font-bold'
                      : isDark ? 'text-slate-300 hover:text-white hover:bg-slate-900' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100'
                  }`}
                >
                  <Icon className={`h-3.5 w-3.5 ${isActive ? 'text-sky-500' : 'opacity-60'}`} />
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              ...
              className="hidden sm:flex items-center space-x-1.5 bg-sky-600 hover:bg-sky-500 text-white font-bold px-3.5 py-2 rounded-lg text-xs transition-all shadow-sm"
            >
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                key={item.id}
                onClick={() => scrollToSection(item.id)}
                className="w-full flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm hover:bg-slate-800/20"
              >
